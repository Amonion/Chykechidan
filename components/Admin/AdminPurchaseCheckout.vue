<template>
  <div
    @click.self="showCart"
    class="checkout-wrap"
    :class="{ active: isShowingPurchaseCart }"
  >
    <div class="checkout-child">
      <div class="das-tb-flow checkout">
        <div class="das-tb checkout">
          ...
          <div
            class="das-tb-body"
            v-for="(item, int) in cartPurchases"
            :key="int"
            :class="{ even: int % 2 == 0 }"
          >
            <div class="das-tb-cell _40">{{ item.name }}</div>
            <div class="das-tb-cell _30">
              {{ formatNumber(item.buyingUnitPrice * 1) }}
            </div>
      ...
      <div class="checkout-info">
        <div class="checkout-text-info">
          Total Items: {{ purchaseProperties.totalQuantity }}
        </div>
        <div class="checkout-text-info">
          Total Amount: N{{ formatNumber(purchaseProperties.totalAmount * 1) }}
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    removeFromCart(data) {
      this.$store.commit("productStore/REMOVE_FROM_PURCHASE", data);
    },

    addToCart(data) {
      this.$store.commit("productStore/ADD_TO_PURCHASE", data);
    },
  ...
  computed: {
    cartPurchases() {
      return this.$store.state.productStore.cartPurchases;
    },
    ...
    isShowingPurchaseCart() {
      return this.$store.state.productStore.isShowingPurchaseCart;
    },
  },
};
</script>
