  ...
  <div class="das-main-body">
    ...
    <div class="das-tb-flow">
      <div class="das-tb">
        ...
        >
          ...
          <div class="das-tb-cell _30">{{ item.greetings }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="das-form">
      <div class="w-layout-grid das-rid">
        ...
        <div class="eac-form-input">
          <div class="from-rid-label">Greetings</div>
          <input
            class="from-rid-input w-input"
            v-model="greetings"
            placeholder="Enter title"
...
export default {
  ...
  methods: {
    ...
    toggleEmail(int) {
      this.$store.commit("admin/TOGGLE_EMAIL", int);
    },
    ...
    copyData(email) {
      this.greetings = email.greetings;
    ...
    async processEmail() {
      ...
      const payload = {
        ...
        url: !this.editState
          ...
          : `/emails/?id=${this.editId}&limit=${this.limit}&page=${this.currentPage}`,
      };

      const result = await this.$store.dispatch("admin/MAKE_POST", payload);
      if (!result.response) {
        this.$store.commit("admin/SET_EMAILS", result.data);
        this.onRequest = false;
        this.clearInputs();
      } else {
